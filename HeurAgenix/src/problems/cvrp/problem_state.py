...
def get_instance_problem_state(instance_data: dict) -> dict:
    ...
    constraint_profile = profile_from_config(instance_data.get("constraint_profile") or instance_data.get("constraints"))
...
def get_solution_problem_state(instance_data: dict, solution: Solution) -> dict:
    ...
    vehicle_tw_violations = []
    total_current_cost = 0
    for vehicle_index in range(vehicle_num):
        route = solution.routes[vehicle_index]
        metrics = compute_route_metrics(route, vehicle_index, instance_data, constraint_profile)
        total_current_cost += metrics.distance
        ...
        vehicle_loads.append(metrics.load)
        # The remaining capacity for each vehicle.
        vehicle_remaining_capacity.append(metrics.remaining_capacity)
        vehicle_waiting_times.append(metrics.waiting_time)
        vehicle_tw_violations.append(metrics.time_window_violations)

    # Calculate route lengths and costs
    route_lengths = [len(route) for route in solution.routes]
    route_costs = [
        compute_route_metrics(route, vehicle_index, instance_data, constraint_profile).distance
    ...
    total_demand_served = sum(demands[node] for route in solution.routes for node in route if node != depot)
    ...
    if constraint_profile.time_windows:
        result.update({
            "vehicle_waiting_times": vehicle_waiting_times,
            "vehicle_tw_violations": vehicle_tw_violations,
            "total_waiting_time": sum(vehicle_waiting_times),
            "total_tw_violations": sum(vehicle_tw_violations),
        })
    return result
